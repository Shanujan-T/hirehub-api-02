...
class Job(db.Model):
    ...
    def to_dict(self, include_employer=False, strip_employer=False):
        data = {
            "id": self.id,
        ...
        if not strip_employer:
            data["employer_id"] = self.employer_id
        if include_employer and self.employer and not strip_employer:
            data["employer"] = {
                "id": self.employer.id,
                "full_name": self.employer.full_name,
                "location": self.employer.location,
            }
        if self.category:
