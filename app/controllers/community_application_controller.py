...
def apply_to_job(job_id, community_id, user_id):
    ...
    application = CommunityApplication(job_id=job_id, community_id=community_id, status="applied")
    db.session.add(application)
    try:
        db.session.commit()
        return jsonify({"message": "Applied to job.", "community_application": application.to_dict(include_community=True)}), 201
    except Exception:
        db.session.rollback()
        return jsonify({"error": "Failed to apply to job."}), 500


def get_applications_for_job(job_id, employer_id):
    job = Job.query.get(job_id)
    if not job:
        return jsonify({"error": "Job not found."}), 404
    ...
    for app in applications:
        ...
        if community:
            ...
            app_data["community"]["members"] = [
                m.to_dict(include_user=True) for m in members
            ]
...
def reject_community(application_id, employer_id):
    application = CommunityApplication.query.get(application_id)
    if not application:
        return jsonify({"error": "Community application not found."}), 404
    job = Job.query.get(application.job_id)
